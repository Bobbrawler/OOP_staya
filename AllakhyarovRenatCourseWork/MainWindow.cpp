...
void MainWindow::on_action_Add_triggered()
{
    ...
    if (addRecordDialogWindow.exec() == QDialog::Accepted)
    {
        ...
        ui->tableWidget->setItem(newRowIndex, 2, new QTableWidgetItem(newRecord.getGroup()));
        ui->tableWidget->setItem(newRowIndex, 3, new QTableWidgetItem(newRecord.getCourse()));
        ui->tableWidget->setItem(newRowIndex, 4, new QTableWidgetItem(newRecord.getFaculty()));
...
void MainWindow::on_action_Edit_triggered()
{
    ...
    editRecordDialogWindow.setGroup(selectedItemsList.at(2)->text());
    editRecordDialogWindow.setCourse(selectedItemsList.at(3)->text());
    editRecordDialogWindow.setFaculty(selectedItemsList.at(4)->text());
    //Внутри этого оператора выполняются действия, пока не будет закрыто окно
    if (editRecordDialogWindow.exec() == QDialog::Accepted) {
        ...
        main_vector[selectedItemsList.at(2)->row()].setGroup(editedRecord.getGroup());
        main_vector[selectedItemsList.at(3)->row()].setCourse(editedRecord.getCourse());
        main_vector[selectedItemsList.at(4)->row()].setFaculty(editedRecord.getFaculty());
        ...
        selectedItemsList.at(2)->setText(editedRecord.getGroup());
        selectedItemsList.at(3)->setText(editedRecord.getCourse());
        selectedItemsList.at(4)->setText(editedRecord.getFaculty());
...
void MainWindow::on_action_Open_triggered(bool isCleanCurrentTable)
{
    ...
    while (!in.atEnd()) {
        ...
        record.setGroup(fields[2].simplified());
        record.setCourse(fields[3].simplified());
        record.setFaculty(fields[4].simplified());
        ...
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 2, new QTableWidgetItem(record.getGroup()));
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 3, new QTableWidgetItem(record.getCourse()));
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 4, new QTableWidgetItem(record.getFaculty()));
...
void MainWindow::on_action_Save_triggered()
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        QString group = ui->tableWidget->item(i, 2)->text();
        QString course = ui->tableWidget->item(i, 3)->text();
        QString faculty = ui->tableWidget->item(i, 4)->text();

        out << recordNumber << DELIMITER << fullName << DELIMITER << group << DELIMITER << course << DELIMITER << faculty << "\n";
    }
...
void MainWindow::on_action_ShowData_triggered()
{
    ...
    for (auto &record : main_vector) {
        recordsInString += record.getFullName() + DELIMITER + record.getGroup() + DELIMITER + record.getCourse() + DELIMITER + record.getFaculty() + "\n";
    }
