...
void MainWindow::on_action_Add_triggered()
{
    ...
    if (addRecordDialogWindow.exec() == QDialog::Accepted)
    {
        ...
        ui->tableWidget->setItem(newRowIndex, 2, new QTableWidgetItem(newRecord.getAddress()));
        ui->tableWidget->setItem(newRowIndex, 3, new QTableWidgetItem(newRecord.getOpeningDate()));
        ui->tableWidget->setItem(newRowIndex, 4, new QTableWidgetItem(newRecord.getArtisticDirection()));
...
void MainWindow::on_action_Edit_triggered()
{
    ...
    editRecordDialogWindow.setAddress(selectedItemsList.at(2)->text());
    editRecordDialogWindow.setOpeningDate(selectedItemsList.at(3)->text());
    editRecordDialogWindow.setArtisticDirection(selectedItemsList.at(4)->text());
    //Внутри этого оператора выполняются действия, пока не будет закрыто окно
    if (editRecordDialogWindow.exec() == QDialog::Accepted) {
        ...
        main_vector[selectedItemsList.at(2)->row()].setAddress(editedRecord.getAddress());
        main_vector[selectedItemsList.at(3)->row()].setOpeningDate(editedRecord.getOpeningDate());
        main_vector[selectedItemsList.at(4)->row()].setArtisticDirection(editedRecord.getArtisticDirection());
        ...
        selectedItemsList.at(2)->setText(editedRecord.getAddress());
        selectedItemsList.at(3)->setText(editedRecord.getOpeningDate());
        selectedItemsList.at(4)->setText(editedRecord.getArtisticDirection());
...
void MainWindow::on_action_Open_triggered(bool isCleanCurrentTable)
{
    ...
    while (!in.atEnd()) {
        ...
        record.setName(fields[1].simplified());
        record.setAddress(fields[2].simplified());
        record.setOpeningDate(fields[3].simplified());
        record.setArtisticDirection(fields[4].simplified());
        ...
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 1, new QTableWidgetItem(record.getName()));
        ui->tableWidget->item(ui->tableWidget->rowCount() - 1, 0);
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 2, new QTableWidgetItem(record.getAddress()));
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 3, new QTableWidgetItem(record.getOpeningDate()));
        ui->tableWidget->setItem(ui->tableWidget->rowCount() - 1, 4, new QTableWidgetItem(record.getArtisticDirection()));
...
void MainWindow::on_action_Save_triggered()
{
    ...
    for (int i = 0; i < len; ++i) {
        ...
        QString Address = ui->tableWidget->item(i, 2)->text();
        QString openingDate = ui->tableWidget->item(i, 3)->text();
        QString artisticDirection = ui->tableWidget->item(i, 4)->text();

        out << recordNumber << DELIMITER << name << DELIMITER << Address << DELIMITER << openingDate << DELIMITER << artisticDirection << "\n";
    }
...
void MainWindow::on_action_ShowData_triggered()
{
    ...
    for (auto &record : main_vector) {
        recordsInString += record.getName() + DELIMITER + record.getAddress() + DELIMITER + record.getOpeningDate() + DELIMITER + record.getArtisticDirection() + "\n";
    }
